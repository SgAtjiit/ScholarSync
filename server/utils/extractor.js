 ...
 */
const extractPageContent = async (config, pageImage, fileName, pageNum) => {
...
DO NOT add any commentary. Just extract the content as it appears.`;
    ...
    try {
        const response = await chatCompletion({
            ...
            model: config.visionModel || process.env.GROQ_VISION_MODEL || 'llama-3.2-90b-vision-preview',
            messages: [{
                role: 'user',
                content: [
 ...
 */
export const extractTextFromMaterials = async (materials, accessToken, llmConfigOrApiKey) => {
    ...
    const config = typeof llmConfigOrApiKey === 'object' ? llmConfigOrApiKey : {
        provider: 'groq',
        apiKey: llmConfigOrApiKey,
    ...
    const textModel = config.textModel || process.env.GROQ_MODEL || 'llama-3.3-70b-versatile';
    const visionModel = config.visionModel || process.env.GROQ_VISION_MODEL || 'llama-3.2-90b-vision-preview';
    ...
    for (const mat of materials) {
        ...
        let downloadResult;
        let fileBuffer;
        if (mat.isManualFile || file.isManualFile) {
            try {
                const storedFileName = file.storedFileName || `${file.id}.pdf`;
                ...
                downloadResult = { success: true, data: fileBuffer, fileName: title };
            } catch (err) {
                failedFiles.push({ fileName: title, error: `Supabase download failed: ${err.message}` });
                continue;
            }
        } else {
            downloadResult = await downloadFile(file.id, accessToken, title);
            ...
            if (mimeType.includes('pdf') || title.toLowerCase().endsWith('.pdf')) {
                ...
                if (pageImages.length === 0) {
                    ...
                    for (const pageImg of pageImages) {
                        const pageContent = await extractPageContent(config, pageImg, title, pageImg.pageNumber);
                ...
                const imageContent = await extractPageContent(config, { dataUrl }, title, 1);
                fullExtractedContent += `\n\n========== ${title} ==========\n${imageContent}`;
