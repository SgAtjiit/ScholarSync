...
import { LogOut, BookOpen, Menu, X, AlertTriangle, Sparkles, RefreshCw } from "lucide-react";
...
const Navbar = () => {
  const { user, logout, connectClassroom } = useAuth();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  
  const handleLogout = async () => {
    const toastId = toast.loading("Logging out & revoking connections...");
    await logout();
    ...
    <nav className="h-14 sm:h-16 border-b border-white/5 bg-[#09090b]/80 backdrop-blur-md sticky top-0 z-50 px-3 sm:px-4 md:px-8">
          ...
          <>
            ...
            <div className="hidden md:flex items-center gap-4">
              {user.hasClassroomConnected ? (
                <span className="flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 text-emerald-400 text-xs font-semibold rounded-lg border border-emerald-500/20">
                ...
                <Button 
                  ...
                  className="bg-indigo-600/10 text-indigo-400 border-indigo-500/20 hover:bg-indigo-600/20 hover:text-white"
                >
                  <RefreshCw size={14} className="mr-1.5" /> Sync Classroom
                </Button>
              )}
      ...
      {user && mobileMenuOpen && (
        <div className="md:hidden absolute top-14 left-0 right-0 bg-[#09090b]/95 backdrop-blur-lg border-b border-white/5 p-4 space-y-3 animate-fade-in">
          ...
          {user.hasClassroomConnected ? (
            <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
              Google Classroom Connected
            </div>
          ) : (
            <button
              onClick={() => { connectClassroom(); setMobileMenuOpen(false); }}
              className="w-full flex items-center gap-3 p-3 rounded-xl bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-600/20 transition-all font-medium text-sm"
            >
              <RefreshCw size={18} /> Sync Google Classroom
            </button>
          )}
